...
// ————————————————— Config par défaut —————————————————
// ⚠️ Les valeurs peuvent être saisies en fraction (0.2 = 20 %) ou en % (20 = 20 %)
export const PRICING_CFG = {
  ...
  charm: true,         // arrondi psychologique .99
  step: 0.01,          // pas d’arrondi final (peut être 0.05 / 0.10 / 0.50 / 1)
};
 ...
 * Retourne un biais (0..0.15) qui sera ajouté à maxChangePct (vers le bas).
 */
...
export function applyGuardrails(Pstar, opts = {}) {
  ...
  const downLimit = base * (1 - (Number.isFinite(downPctWithBias) ? downPctWithBias : 0) / 100);

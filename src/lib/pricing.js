// src/lib/pricing.js
import { applyGuardrails as applyGuardrailsLib, PRICING_CFG } from "./pricingGuardrails";
 ...
 */
export function estimateElasticityLogLog(samples) {
  // samples = [{ price, qty }, ...] min 4 pts idéalement
  if (!samples || samples.length < 4) return null;
  const xs = samples.map(s => Math.log(Math.max(s.price, 1e-6)));
  const ys = samples.map(s => Math.log(Math.max(s.qty, 1e-6)));
  const n = xs.length;
  const mean = arr => arr.reduce((a,b)=>a+b,0)/arr.length;
  const mx = mean(xs), my = mean(ys);
  let num=0, den=0;
  for (let i=0;i<n;i++){ num += (xs[i]-mx)*(ys[i]-my); den += (xs[i]-mx)**2; }
  const beta1 = den === 0 ? 0 : num/den;  // ~ élasticité (ε)
  return beta1; // attendu < 0
}
...
export function optimalPrice({ c, e, P0 }) {
  ...
  // évite valeurs bizarres sous le coût
  if (!Number.isFinite(Pstar) || Pstar <= 0) return Math.max(cost, toNum(P0, cost));
  return Pstar;
}

// Variante pratique quand on a |e|
export function optimalPriceLerner(cost, eAbs) {
  // p* = c * (|ε|/(|ε|-1))  ; condition |ε|>1
  if (!isFinite(cost) || cost <= 0 || !isFinite(eAbs) || eAbs <= 1) return null;
  return cost * (eAbs/(eAbs - 1));
}
...
// ——— 4) Reco unique (Lerner + garde-fous centralisés) ———
/**
 * recommendPrice({
 *   cost, priceCurr, samples, category, isKVI,
 *   markdownContext,   // { stock_on_hand, last_90d_qty, lead_time_days }  (optionnel)
 *   cfg                // facultatif, par défaut PRICING_CFG
 * })
 */
export function recommendPrice({
  ...
  let e = estimateElasticityLogLog(samples);
  if (e == null) {
    e = cfg?.defaultElasticityByCategory?.[category] ??
        cfg?.defaultElasticityByCategory?.autre ?? -0.8;
  }
  const eAbs = Math.abs(e);
  const pStar = eAbs > 1 ? optimalPriceLerner(cost, eAbs) : null;

  // applique les garde-fous de la lib (un seul applyGuardrails dans le projet)
  const pRec = applyGuardrailsLib(pStar ?? priceCurr, {
    P0: priceCurr,
    cost,
    isKVI: !!isKVI,
    // on laisse la lib lire ses propres defaults; on lui passe ce qu'on a:
    step: cfg.step,
 ...
 *   cost: number,              // c
 *   guardrails: { step, maxChangePct, minMarginPct, minPrice, maxPrice } // (optionnel; lib prend ses defaults)
 * })
...
export function suggestPrice({
  ...
  const suggested = applyGuardrailsLib(P_theory, { P0, cost, ...guardrails });
